...
class TwoPieceAutoTwo(yeti.Module):
    """
    This is one possible version of a two piece autonomous mode. It simultaneously drives forward and left,
    and then strafes right. It averages about 2.5 seconds for the sequence and should work in all staging zones.
    It runs the risk of tipping containers, though.
    """
    ...
    def two_piece_run(self):
        ...
        self.check_mode()
        call_public_method("elevator.goto_pos", 3)
        self.check_mode()

        # Strafe around container
        self.logger.info("Drive phase 1")
        self.drivetrain_setpoint_datastream.push({"x_pos": -2.3, "y_pos": 4})
        yield from call_public_coroutine("drivetrain.wait_for_x")
        while self.drivetrain_sensor_input.get().get("y_pos") < 3:
            yield from asyncio.sleep(.1)
            self.check_mode()
        self.check_mode()
        ...
        self.drivetrain_setpoint_datastream.push({"x_pos": 8, "r_pos": 10})
