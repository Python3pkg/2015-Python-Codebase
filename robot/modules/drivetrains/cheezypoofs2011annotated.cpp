...
void ContinuousAccelFilter::maxAccelTime(double distance_left, double curr_vel, double goal_vel, double max_a, double max_v, double& t1, double& a1, double& ct, double& t2, double& a2)
{
    ...
    //---------------------------------max_accel_velocity: The velocity I would be travelling if I were to accelerate at max acceleration until there were no distance left
    ...
    //---------------------------------set final acceleration to +-max_a depending on if max_accel_velocity is too fast or too slow
    ...
    // DISSECTED, OBFUSCATED VERSION

    // var_aa is the distance required to go 0-to-curr_vel at acceleration start_a
    double var_aa = (curr_vel * curr_vel) / (2.0 * start_a)

    // var_ab is the distance required to go 0-to-goal_vel at acceleration final_a
    double var_ab = (goal_vel * goal_vel) / (2.0 * final_a)

    // var a is the distance required to do the following: travel distance_left, decelerate to 0, and accelerate to goal_vel
    double var_a = distance_left + var_aa + var_ab

    // var ba is
    double var_ba = 1.0 / (2.0 * start_a)
    double var_bb = 1.0 / (2.0 * final_a)
    double var_b = var_ba - var_bb

    double top_v = sqrt(var_a / var_b)
